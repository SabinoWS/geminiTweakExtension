/**
 * Feature: Esconder Meus Itens
 * Dependências: SidebarPage PageObject
 */

function addMyItemsControls() {
    if (!featureMyItemsEnabled) return;

    const targetButton = SidebarPage.getMyItemsButton();

    if (targetButton && !targetButton.querySelector('.gemini-my-items-controls')) {
        const titleContainer = SidebarPage.getMyItemsTitleContainer(targetButton);
